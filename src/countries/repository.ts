...
const getCountriesByContinentIdsMap = async (ids: number[]): Promise<Map<number, Country[]>> => {
    const [results] = await connection.execute<CountryDB[]>(`SELECT *
                                                           FROM countries
                                                           WHERE continent_id IN (${placeholderIds(ids)})`)
...
const getCountryById = async (countryId: number): Promise<Country[]> => {
    const [result] = await connection.execute<CountryDB[]>(`SELECT * FROM countries WHERE country_id = (${placeholderSingle})`, [countryId])
...
const checkCountryExists = async (name: string): Promise<any> => {
    const [result] = await connection.execute(`SELECT EXISTS(SELECT * FROM countries WHERE name = (${placeholderSingle})) AS countryExists`, [name])
