...
import {placeholderIds, placeholderSingle} from "../utils/database";
...
const getCityById = async (cityId: number): Promise<City[]> => {
    const [result] = await connection.execute<CityDB[]>(`SELECT * FROM cities WHERE city_id = (${placeholderSingle})`, [cityId])
...
const checkCityExists = async (name: string): Promise<any> => {
    const [result] = await connection.execute(`SELECT EXISTS(SELECT * FROM cities WHERE name = (${placeholderSingle})) AS cityExists`, [name])
...
const getCitiesByIds = async(ids: number[]): Promise<City[]> => {
    const [results] = await connection.execute<CityDB[]>(`SELECT * FROM cities WHERE cities.city_id IN (${placeholderIds(ids)})`)
    return results.map(cityDB => {
        const city: City = {
            cityId: cityDB.city_id,
            countryId: cityDB.country_id,
